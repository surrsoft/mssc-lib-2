...
import { DisEnum, InfoEnum } from './types';

describe("indexDiapHelper", () => {
  it("standard", () => {
    const result = indexesDiapAdapter(0, 1, 2);
    expect(result._tag).toEqual(DisEnum.BASE);
    if (result._tag === DisEnum.BASE) {
  ...
  it("len 0", () => {
    ...
    expect(result._tag).toEqual(DisEnum.EMPTY_ARR);
  });
  ...
  it("indexStart -2", () => {
    ...
    if (result._tag === DisEnum.BASE) {
      ...
      expect(result.infos).toEqual(
        expect.arrayContaining([InfoEnum.INDEX_START_LESS_NIL])
      );
    }
  });
  ...
  it("indexEnd out of range", () => {
    ...
    if (result._tag === DisEnum.BASE) {
      ...
      expect(result.infos).toHaveLength(1);
      expect(result.infos).toEqual(
        expect.arrayContaining([InfoEnum.INDEX_END_GREAT_LEN])
      );
    }
  });
  ...
  it("indexEnd < indexStart", () => {
    ...
    if (result._tag === DisEnum.BASE) {
      ...
      expect(result.infos).toHaveLength(1);
      expect(result.infos).toEqual(
        expect.arrayContaining([InfoEnum.END_GREAT_START])
      );
    }
  });
  ...
  it("indexEnd === indexStart", () => {
    ...
    if (result._tag === DisEnum.BASE) {
      ...
      expect(result.infos).toHaveLength(1);
      expect(result.infos).toEqual(
        expect.arrayContaining([InfoEnum.END_EQUAL_START])
      );
    }
  });
  ...
  it("indexStart > len", () => {
    ...
    if (result._tag === DisEnum.BASE) {
      ...
      expect(result.infos).toHaveLength(1);
      expect(result.infos).toEqual(
        expect.arrayContaining([InfoEnum.INDEX_START_GREAT_OR_EQ_LEN])
      );
    }
  });
